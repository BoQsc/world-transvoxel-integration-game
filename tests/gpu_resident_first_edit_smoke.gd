...
func _run() -> void:
	...
	for _frame in range(900):
		...
		for state_value in _world.get_debug_gpu_processing_states():
			...
			if state.get("stage", "") == "visible" and int(identity.get("lod", 0)) > 0:
				coarse = true
		if coarse and bool(_world.get_cold_idle_summary().get("cold_idle", false)):
			break
	if not coarse:
...
func _fail(message: String) -> void:
	push_error("GPU_RESIDENT_FIRST_EDIT_SMOKE_FAIL: " + message)
	quit(1)
