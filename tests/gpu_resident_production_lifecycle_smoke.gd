...
func _run() -> void:
	...
	if not bool(status.get("running", false)) \
			...
			or bool(effect_status.get("compacted_surface_indirect_commands", true)) \
			or int(effect_status.get("indirect_commands_per_surface", 0)) != 32 \
			or int(effect_status.get("asynchronous_summary_bytes", 0)) != 20 \
			...
			or int(effect_status.get("max_compact_command_records_per_view", 0)) > 6 \
