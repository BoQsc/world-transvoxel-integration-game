@tool
extends Node
class_name WtGameTerrainMaterialApplicator

const TERRAIN_SHADER := preload("res://addons/world_transvoxel_gameworld/material/wt_game_terrain_palette.gdshader")
const WATER_SHADER := preload("res://addons/world_transvoxel_gameworld/material/wt_game_static_water.gdshader")
...
func _procedural_ore_blend_enabled(generation: Dictionary) -> bool:
	return str(generation.get("source_mode", "")) == "DETERMINISTIC_REFERENCE" and \
		str(generation.get("procedural_preset_id", "")) in [
			"rolling_hills_cave",
...
func _procedural_road_blend_enabled(generation: Dictionary) -> bool:
	...
	return preset in ["rolling_hills_cave_roads", "four_biomes_lakes_caves_roads"]
