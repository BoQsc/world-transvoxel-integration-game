...
| Materials and biomes | Human terrain material uses native render material override, mipmapped imported texture cache, texture-array material layers, native surface biome blend weights, and world-space triplanar production texture mapping so dug vertical walls do not smear from `XZ`-only UV projection. Material-ID tinting is diagnostic only; it must not become the default because it produces contour-like bands across normal terrain. The active material/biome standard defines grass, sand, gravel, snow, stone, rock, and ore patch IDs. | P2 proof fields: `material=1`, `production_texture_active=1`, `native_render_material_override=1`; import gate passes; human marker `20260715T012849_001_human` is the regression sample for vertical texture stretch; material-marker classifier includes `sand_triplanar`, `flat_clean`, `material_tint`, and `production_texture_array`; see `MATERIAL_AND_BIOME_STANDARD.md`. |
...
production texture array. If the artifact disappears in flat color, classify it
as material/texture presentation until a geometry probe proves otherwise.
