#!/usr/bin/env python3
"""Run the bounded resident multi-chunk relocation smoke on three CPUs."""
...
PASS_MARKER = "GPU_RESIDENT_MULTICHUNK_RELOCATION_SMOKE_PASS"
...
def run_profile(godot: pathlib.Path, project: pathlib.Path, driver: str) -> int:
    log_root = (
        ...
        / "gpu_resident_multichunk_relocation"
    )
    ...
    command = [
        ...
        str(log_path),
        "--script",
        "res://tests/gpu_resident_multichunk_relocation_smoke.gd",
    ]
    exit_code = run_with_affinity(command, project, 180)
    output = log_path.read_text(encoding="utf-8", errors="replace")
    relevant = [
        line
        for line in output.splitlines()
        if "GPU_RESIDENT_MULTICHUNK_RELOCATION" in line
